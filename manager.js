...
function renderCards(){
  ...
  var pgGrandC=pgC+jfTotal+hlCost;
  pgH+=hlHrs;
  ...
  // Holiday card(s) — one per holiday in view, listing each billed unit
  var hlCardHtml=holidays.map(function(h){
    return '<div class="driver-group" style="border:1.5px solid #059669;border-radius:8px;margin-bottom:10px;overflow:hidden">'
      +'<div style="background:#ecfdf5;padding:12px 14px;display:flex;align-items:center;justify-content:space-between">'
      +'<div style="font-family:Barlow Condensed,sans-serif;font-size:17px;font-weight:700">&#127881; Holiday &mdash; '+holidayLabel(h)+' <span style="font-size:13px;font-weight:400;color:var(--muted)">('+h.units.length+' units &times; '+HOLIDAY_HOURS+' hrs)</span></div>'
      +'<div style="font-family:Barlow Condensed,sans-serif;font-size:20px;font-weight:800;color:#059669">$'+h.cost.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})+'</div>'
      +'</div>'
      +'<div style="padding:10px 14px;font-size:12px;color:var(--text2)">'
      +h.units.map(function(u){
        return '<div style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border)">'
          +'<span><strong>'+u.unit+'</strong> &nbsp;·&nbsp; '+u.name+' &nbsp;·&nbsp; '+u.hours+' hrs @ $'+u.rate+'/hr</span>'
          +'<span style="font-weight:700;color:#059669">$'+u.cost.toFixed(2)+'</span>'
          +'</div>';
      }).join('')
      +'</div>'
      +'</div>';
  }).join('');
  c.innerHTML=totalsBox+driverCardsHtml+hlCardHtml+jfCardHtml;
}
