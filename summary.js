...
function renderRangeStats(){
  ...
  holidays.forEach(function(h){gH+=h.hours;gC+=h.cost;});
  var hlRows=holidayRowsHtml(holidays,'days');
...
function dlRangeReport(){
  ...
  holidays.forEach(function(h){
    csv+='"Holiday - '+holidayLabel(h)+' - '+h.units.length+' units x '+HOLIDAY_HOURS+' hrs",,,,,,,'+h.hours.toFixed(2)+',$'+h.cost.toFixed(2)+'\n';
    gH+=h.hours;gC+=h.cost;
  });
...
function renderCustomerDash(){
  ...
  // Holiday charges (flat 8 hrs per real unit) — see getHolidayCharges()
  var holidays=getHolidayCharges(sunday,friday);
  var hlRows=holidayRowsHtml(holidays,'blank');
  holidays.forEach(function(h){gH+=h.hours;});
  var hlCost=holidays.reduce(function(s,h){return s+h.cost;},0);
  var grandC=gC+jfTotal+hlCost;
  var grandW=gW+jfWt;
  ...
  el.innerHTML=
    ...
    +'<tfoot>'+hlRows+jfRow+'<tr class="total-row"><td colspan="2"><strong>TOTAL</strong></td><td>'+gD+'</td><td>'+gP+'</td><td>'+gS+'</td><td>'+grandW.toLocaleString()+'</td><td>'+gM+'</td><td>'+gH.toFixed(2)+'</td><td class="chg-cell">$'+grandC.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})+'</td><td></td></tr></tfoot>'
    +'</table></div>'
    +'</div>';
}
...
// ── HOLIDAY ROWS ──────────────────────────────────────────────────────────────
// One table row per holiday in range. Uses a colspan="2" label cell (Unit +
// Driver) so printSummary()'s Driver-column removal handles it the same way
// it handles the J Files and TOTAL rows. `cols` = number of cells after the
// Hours/Charges pair: 'cpl' adds a $/Lb dash, 'blank' adds an empty cell,
// 'days' puts a dash in a Days column before Charges.
function holidayRowsHtml(holidays,layout){
  return holidays.map(function(h){
    var lbl='<td colspan="2" style="white-space:normal;overflow:visible;text-overflow:clip"><strong>&#127881; Holiday</strong> &mdash; '+holidayLabel(h)+'<br><span style="font-size:11px;font-weight:400;color:#666">'+h.units.length+' units &times; '+HOLIDAY_HOURS+' hrs</span></td>';
    var chg='<td class="chg-cell" style="color:#059669">$'+h.cost.toLocaleString('en-US',{minimumFractionDigits:2,maximumFractionDigits:2})+'</td>';
    var base='<td>—</td><td>—</td><td>—</td><td>—</td><td>—</td><td>'+h.hours.toFixed(2)+'</td>';
    var tail=layout==='cpl'?chg+'<td>—</td>':layout==='blank'?chg+'<td></td>':layout==='days'?'<td>—</td>'+chg:chg;
    return '<tr class="holiday-row" style="background:#ecfdf5">'+lbl+base+tail+'</tr>';
  }).join('');
}
...
function renderSum(){
  ...
  // Holiday charges (flat 8 hrs per real unit) — see getHolidayCharges()
  var holidays=getHolidayCharges(sunday,friday);
  var hlRows=holidayRowsHtml(holidays,'cpl');
  var hlHrs=holidays.reduce(function(s,h){return s+h.hours;},0);
  var hlCost=holidays.reduce(function(s,h){return s+h.cost;},0);
  var workedH=gH; // shipments/hr stays based on hours actually worked
  gH+=hlHrs;
  var grandC=gC+jfTotal+hlCost;
  ...
  el.innerHTML=
    ...
    +'<tbody>'+ttRows+jfRow+stRows+hlRows+'</tbody>'
...
function dlWeekly(){
  ...
  getHolidayCharges(sunday,friday).forEach(function(h){
    rows.push(['Holiday','"'+holidayLabel(h)+' - '+h.units.length+' units x '+HOLIDAY_HOURS+' hrs"','','','','','',h.hours.toFixed(2),'$'+h.cost.toFixed(2)].join(','));
  });
  var csv=rows.join('\n');
  var blob=new Blob([csv],{type:'text/csv'});
  var url=URL.createObjectURL(blob);
